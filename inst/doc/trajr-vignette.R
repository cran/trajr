...
## ----randomWalk, echo=FALSE, fig.height=4, fig.width=6, fig.cap="_A random walk with 5 steps_"----
...
## ----directedWalk, echo=FALSE, fig.height=4, fig.width=6, fig.cap="_A directed walk with 5 steps_"----
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
...
## ----creation, fig.width=6, fig.height=4, fig.cap="_Trajectory created from coordinates_", echo=-1----
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
...
## ---- eval=FALSE---------------------------------------------------------
#  # Example 1
#  coords <- read.csv("H.pahangensis_096xypts.csv")
...
#  
#  # Example 2
#  coords <- read.csv("A.argentifasciata_C0139xypts.csv")
#  # In this video, a 20 cm (= .2 m) object had length 1800 pixels, scale to metres
#  trj <- TrajScale(trj, .2 / 1800, "m")

## ----smooth, fig.width=6, fig.height=4, fig.cap="_Effect of smoothing a trajectory_", echo=-1:-2----
set.seed(3)
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
...
## ----speed, fig.cap="_Speed and acceleration over time_", fig.width=6, fig.height=4, echo=-1:-2----
set.seed(1)
...
## ----speedIntervals, fig.cap="_Speed over time, hovering intervals (speed < 2 m/s) highlighted_", fig.width=6, fig.height=4, echo=-1:-2----
set.seed(1)
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
...
## ----straightness, fig.cap="_Two straightness indices (r, dots, and D/L, crosses) as a function of angular error, plotted for several different step sizes (linear axes)_", fig.height=4, fig.width=6, echo=-1----
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
...
plot(rep(angularErrorSd, 3), rs,
     ...
     xlab = expression(sigma["angular error"]), ylab = "Straightness",
     ylim = range(c(ds, rs)))
...
## ----Sinuosity, fig.cap="_Sinuosity as a function of angular error, plotted for several different step sizes (linear axes)_", fig.height=4, fig.width=6, echo=-1----
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
...
## ----emax, fig.cap="_E~max~ as a function of angular error (logarithmic axes)_", fig.height=4, fig.width=6, echo=-1----
...
## ----fractal, fig.cap="_Fractal dimension as a function of angular error_", fig.width=6, fig.height=4, echo=-1----
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
...
## ----DC, fig.cap="_DC and SDDC as a function of angular error_", fig.height=4, fig.width=6, echo=-1----
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
...
## ----dirnAuto, fig.cap="_Direction autocorrelation of a random trajectory with first local minimum_", fig.width=6, fig.height=4, echo=c(-1,-2)----
set.seed(1)
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
...
## ----pca, fig.cap="_PCA of trajectory characterisations_", fig.width=6, fig.height=4, echo=-1----
par(mar = c(4, 4, 0.5, 0.5) + 0.1)
#---
